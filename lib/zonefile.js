const defaultTemplate = `; \tZone name: {zone}
...
export let generate = function (options, template) {
  ...
  template = processAlias(json['azureAlias'] || [], template);
...
let processAlias = function(data, template) {
    let ret = '';
    // sortByName(data);
    for (let value of data) {
      ret += (value.name || '@') + '\t';
      if (value.ttl) ret += value.ttl + '\t';
      ret += 'AZURE ALIAS \t'+value.type+'\t' + value.resourceId + '\n';
    }
    return template.replace('{azureAlias}', ret);
};

// Sorts record list by name, as name of DNS record is mandatory field so assumed that it would always be defined. 
let sortByName = function(data) {
...
let processValues = function (options, template) {
    ...
    if (options.$origin !== undefined) {
        zoneName = options.$origin;
    } else if (options.soa !== undefined) {
        zoneName = options.soa.name;
    }
    template = template.replace('{zone}', zoneName);
    template = template.replace('{datetime}', (new Date()).toUTCString());
...
export let removeComments = function (text) {
...
/**
 * Flattens SOA record, convert multi line SOA record to single line SOA
 * By converting everything inside parenthesis into one line (Content remained unchanged) 
 * @param {*} text The input zone file.
 * @returns 
 */
export let flatten = function (text) {
    ...
    if (rr.startsWith('$ORIGIN')) {
      ...
      switch (nrr.rrType) {
        ...
        case 'DS':
          ...
          break;
        case 'AZURE':
          ret.azureAlias = ret.azureAlias || [];
          ret.azureAlias.push(parseAlias(nrr, ret.azureAlias));
          break;
      }
    }
  }
...
let parseAlias = function(rrData, recordsSoFar) {
    let rrTokens = rrData.tokens;
    if (!rrData.hasName) {
      if (recordsSoFar.length) {
        rrTokens.unshift(recordsSoFar[recordsSoFar.length - 1].name);
